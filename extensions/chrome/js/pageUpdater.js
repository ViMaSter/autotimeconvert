...
function Convert(stringTested, p1, p2, p3, p4, p5, p6, p7, p8, p9, p10) {
	...
	if (value[currentIndex-1][0].toLowerCase() == "p" || value[currentIndex-1].toLowerCase() == "a") {
		constructedNumber.IsPM = value[currentIndex-1][0].toLowerCase() == "p";
	} else {
		constructedNumber.IsPM = constructedNumber.Hours >= 12;
	}

	constructedNumber.TimeZone = value[currentIndex];


	var date = Date.parse(constructedNumber.Hours + ":" + constructedNumber.Minutes + ":" + constructedNumber.Seconds + " " + (constructedNumber.IsPM ? "PM" : "AM") + " " + constructedNumber.TimeZone);

	console.log(date.toString("yyyy-MM-ddTHH:mm:ssZ"));
...
function ParsePage() {
	return document.documentElement.innerHTML.replace(TimeExpression, Convert);
}

function UpdatePage() {
	document.documentElement.innerHTML = ParsePage();
}
